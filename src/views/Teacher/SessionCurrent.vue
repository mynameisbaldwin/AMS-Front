    ...
    <div class="teacher-wrapper">
      ...
      <div class="table-session" id="unaccounted">
        ...
        <b-table :items="unaccounted" :fields="fields" :filter="filter">
          <template slot="presence" slot-scope="data">
            ...
            <button class="btn-mark-presence btn-tardy" @click="markPresence(data.item.id, data.item.name, 'Tardy')">T</button>
            <button class="btn-mark-presence btn-absent" @click="markPresence(data.item.id, data.item.name, 'Absent')">A</button>
            <button class="btn-mark-presence btn-present" @click="markPresence(data.item.id, data.item.name, 'Present')">P</button>
          </template>
        </b-table>
      </div>
      <div class = "table-session" id="accounted">
        ...
        <b-table :items="accounted" :fields="fields" :tbody-tr-class="rowClass">
          ...
          <template slot="presence" slot-scope="data">
            {{ data.item.presence }}
            <b-form-select id="change-presence-dropdown" 
...
export default {
  ...
  methods: {
    ...
    markPresence(id, name, presence) {
      ...
      this.unaccounted = this.unaccounted.filter(unaccounted => unaccounted.id != id);
    ...
    cancelSession() {
      //cancels session
      bootbox.confirm({
        message: "Are you sure you want to cancel the current session? All progress will be lost.",
        buttons: {
          confirm: {
            label: 'Yes, Cancel'
          },
    ...
    saveSession() {
      ...
      {
        bootbox.confirm({
          message: "There are still students left unaccounted. Would you like to continue saving anyways? Remaining students will be left unmarked.",
          buttons: {
            confirm: {
          ...
          callback: function(result) 
          {
            if(result) 
            {
              axios.post('https://jsonplaceholder.typicode.com/users', this.accounted)
